...
try {
    ...
    $db->execute(
        'UPDATE blog_posts SET category_id = ?, title = ?, slug = ?, excerpt = ?, content = ?, featured_image = ?, author_name = ?, status = ?, is_featured = ?, seo_title = ?, seo_description = ?, published_at = ?, scheduled_at = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
        'isssssssissssi',
        [$input['category_id'], $input['title'], $input['slug'], $input['excerpt'], $input['content'], $input['featured_image'], $input['author_name'], $input['status'], $input['is_featured'], $input['seo_title'], $input['seo_description'], $input['published_at'], $input['scheduled_at'], $id]
    );
