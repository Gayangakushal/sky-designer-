...
function blog_post_input(array $data): array
{
    ...
    if (!in_array($status, ['draft', 'scheduled', 'published'], true)) {
        blog_fail('Status must be draft, scheduled, or published.', 422);
    }
    ...
    if ($status === 'scheduled') {
        if ($scheduledAt === null || $scheduledAt === '') {
            blog_fail('A scheduled publishing date is required.', 422);
        }
        try {
            $schedule = (new DateTimeImmutable((string) $scheduledAt))->setTimezone($utc);
        } catch (Throwable) {
            blog_fail('Scheduled publishing date is invalid.', 422);
        }
        if ($schedule <= new DateTimeImmutable('now', $utc)) {
            blog_fail('Scheduled publishing date must be in the future.', 422);
        }
