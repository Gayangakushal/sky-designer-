...
function billing_require_admin(array $c): array { if(!function_exists('curl_init'))billing_fail('Server authentication is unavailable.',500);$token=billing_token();$url=rtrim((string)($c['supabase_url']??''),'/');$key=(string)($c['supabase_anon_key']??'');if($url===''||$key==='')billing_fail('Server authentication is not configured.',500);$u=billing_http($url.'/auth/v1/user',$key,$token);if(!$u['ok']||empty($u['data']['id']))billing_fail('Invalid or expired session.',401);$r=billing_http($url.'/rest/v1/rpc/has_role',$key,$token,['_user_id'=>$u['data']['id'],'_role'=>'admin']);if(!$r['ok']||!in_array($r['data'],[true,'true'],true))billing_fail('Administrator access required.',403);return $u['data']; }

$billingUser=billing_require_admin($billingConfig);
