...
import { deleteBlogPost, formatBlogDate, getPostCategoryName, updateBlogPost } from "@/lib/blog-api";
...
const AdminBlog = () => {
  ...
  const changeStatus = async (post: BlogPost, nextStatus: BlogStatus) => {
    try {
      await updateBlogPost(post.id, {
        ...
        status: nextStatus, is_featured: post.is_featured,
                  ...
                  <tbody>
                    {filtered.map((post) => (
                      <tr key={post.id}>
                        ...
                        <td>{post.status === "scheduled" ? formatColomboDateTime(post.scheduled_at) : formatBlogDate(post.published_at)}</td>
                        ...
                        <td>
                          <div className="flex gap-1">
                            ...
                            <button
                              className="admin-row-action"
                              onClick={() => void changeStatus(post, post.status === "published" ? "draft" : "published")}
                              aria-label={post.status === "published" ? `Unpublish ${post.title}` : `Publish ${post.title} now`}
                            >
                              {post.status === "published" ? "Draft" : "Now"}
                            </button>
                            <button
              ...
              <div className="blog-admin-mobile-list">
                {filtered.map((post) => (
                  <article className="blog-admin-mobile-card" key={post.id}>
                    ...
                    <h3>{post.title}</h3>
                    <p>
                      {getPostCategoryName(post)} · {post.status === "scheduled" ? formatColomboDateTime(post.scheduled_at) : formatBlogDate(post.published_at)}
                    </p>
                    <div>
                      <Link to={`/admin/blog/${post.id}/edit`}>
                        <Edit3 size={15} /> Edit
                      </Link>
                      <button onClick={() => void changeStatus(post, post.status === "published" ? "draft" : "published")}>
