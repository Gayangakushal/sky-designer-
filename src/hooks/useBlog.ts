...
export const publishedBlogPostsQuery = () =>
  queryOptions({ queryKey: ["blog", "published"], queryFn: () => fetchPublishedPosts().then((data) => data.posts), staleTime: 60_000 });

export const blogPostQuery = (slug: string) =>
  queryOptions({ queryKey: ["blog", "post", slug], queryFn: () => fetchPostBySlug(slug).then((data) => data.post), staleTime: 60_000 });

export const usePublishedPosts = () => useQuery(publishedBlogPostsQuery());

export const useBlogPost = (slug: string) =>
  useQuery({ ...blogPostQuery(slug), enabled: Boolean(slug), retry: (count, error) => !("status" in error && error.status === 404) && count < 2 });
